...
> - **PARCHE** — correcciones, ajustes menores o refactorizaciones
 ...
 - **[Security] Gestión de Secretos**: Migración completa de credenciales y variables de entorno (`SECRET_KEY`, credenciales de BD, `ALLOWED_HOSTS`) a un archivo `.env` utilizando `python-decouple`, mejorando significativamente la seguridad del entorno.
 - **[Security] Rate Limiting**: Implementación de la librería `django-ratelimit` en el endpoint de verificación de correos (`VerificarEmail`) para mitigar ataques de fuerza bruta (limite: 5 peticiones por minuto por IP).
 - **[Optimization] Prevención de N+1**: Refactorización del endpoint de `PaquetesTuristicos` implementando `select_related` y `prefetch_related` nativos de Django, reduciendo drásticamente las consultas a la base de datos al cargar catálogos.
 - **[Feature] Logging Profesional**: Sustitución de sentencias `print()` en controladores críticos por un sistema de `logging` estructurado con niveles (INFO, ERROR, CRITICAL), permitiendo mejor trazabilidad en entornos de producción.
 - **[Fix] Validación Robusta en Importaciones**: Inyección de reglas de validación explícitas en `CargaMasivaPaquetesAPIView` para interceptar y reportar precios negativos o capacidades inválidas durante la subida masiva de Excel/CSV.
 - **[Doc] API y Changelog**: Integración de `drf-spectacular` para autogeneración de documentación OpenAPI/Swagger UI, y extracción del historial de versiones a un archivo nativo `CHANGELOG.md`.
