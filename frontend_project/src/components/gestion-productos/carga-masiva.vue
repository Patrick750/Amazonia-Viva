...
const descargarPlantilla = async () => {
  try {
    const res = await axios.get('api/productos/carga-masiva/', { responseType: 'blob' });
...
const subirArchivo = async () => {
  ...
  try {
    ...
    const res = await axios.post('api/productos/carga-masiva/', formData, {
      headers: {
          ...
          <div class="relative z-10 flex items-start justify-between gap-4">
            <div>
              ...
              <p class="text-emerald-100/60 text-sm mt-1.5 font-medium italic">
                Sube un archivo Excel o CSV para crear múltiples productos a la vez.
              </p>
            </div>
